...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Evaluate AP-Attack against the DukeMTMC-ReID AlignedReID victim.'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            '/home/lzf/ldx/checkpoints/DeepMisRanking/Aligned/duke/'
...
def extract_aligned_feature(model, images):
...
def extract_query_features(loader, model, generator):
    clean_features, clean_local_features = [], []
    adv_features, adv_local_features = [], []
    pids, camids = [], []
    ...
    for batch_idx, (images, batch_pids, batch_camids, _) in enumerate(loader):
        images = images.cuda()
        clean_feature, clean_local_feature = extract_aligned_feature(model, images)
        clean_features.append(clean_feature.cpu())
        clean_local_features.append(clean_local_feature.cpu())

        adv_images, actual_delta = generate_adversarial_images(generator, images)
        adv_feature, adv_local_feature = extract_aligned_feature(model, adv_images)
        adv_features.append(adv_feature.cpu())
        adv_local_features.append(adv_local_feature.cpu())
    ...
    return (
        ...
        torch.cat(clean_local_features, 0),
        torch.cat(adv_features, 0),
        torch.cat(adv_local_features, 0),
        np.asarray(pids),
        np.asarray(camids),
...
def extract_gallery_features(loader, model):
    features, local_features, pids, camids = [], [], [], []
    for batch_idx, (images, batch_pids, batch_camids, _) in enumerate(loader):
        images = images.cuda()
        feature, local_feature = extract_aligned_feature(model, images)
        features.append(feature.cpu())
        local_features.append(local_feature.cpu())
        pids.extend(batch_pids.cpu().numpy())
        camids.extend(batch_camids.cpu().numpy())
        print('Processed gallery batch {}/{}'.format(batch_idx + 1, len(loader)))
    return (
        torch.cat(features, 0),
        torch.cat(local_features, 0),
        np.asarray(pids),
        np.asarray(camids),
    )
...
def main():
    ...
    with torch.no_grad():
        ...
        gf, lgf, g_pids, g_camids = extract_gallery_features(gallery_loader, model)
