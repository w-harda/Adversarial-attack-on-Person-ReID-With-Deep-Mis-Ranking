...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Evaluate Duke-trained AP-Attack against CUHK03 IDE.'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=(
            '/home/lzf/ldx/checkpoints/DeepMisRanking/IDE/cuhk03/'
            'cuhk03.pth.tar'
        ),
    )
    parser.add_argument(
...
def reid_collate_fn(batch):
    images, pids, camids, pids_raw = zip(*batch)
    return (
...
def main():
    ...
    assert attacked_query_count == 1400
    assert clean_qf.size(1) == 767
    assert adv_qf.size(1) == 767
    assert gf.size(1) == 767
    assert gf.size(0) == len(dataset.gallery)
    assert gf.size(0) == 5332
